 ...
 */
public class SpriteLDAPred extends TopicModel implements Serializable {
	...
	public SpriteLDAPred(int z, double sigmaAlpha0, double sigmaA0, double sigmaAB0, double sigmaW0, double sigmaWB0,
...
//						updateGradient(iter, minZ, maxZ, minW, maxW);
	...
	public void writeOutput(String filename) throws Exception {
		...
		FileWriter fw = new FileWriter(new File(outputDir, baseName + String.format(".lda.pred.%d.assign", predFold)));
		...
		fw = new FileWriter(new File(outputDir, baseName + String.format(".lda.pred.%d.beta", predFold)));
		...
		fw = new FileWriter(new File(outputDir, baseName+String.format(".lda.pred.%d.betaB", predFold)));
		bw = new BufferedWriter(fw);
		for (int z = 0; z < Z; z++) { 
			for (int c = 0; c < Cph; c++) {
				bw.write(betaB[z][c]+" ");
		...
		fw = new FileWriter(new File(outputDir, baseName+String.format(".lda.pred.%d.omega", predFold)));
		bw = new BufferedWriter(fw);
		for (int w = 0; w < W; w++) {
		...
		fw = new FileWriter(new File(outputDir, baseName+String.format(".lda.pred.%d.omegaBias", predFold)));
		bw = new BufferedWriter(fw);
		for (int w = 0; w < W; w++) {
			String word = wordMapInv.get(w);
			bw.write(word);
		...
		fw = new FileWriter(new File(outputDir, baseName+String.format(".lda.pred.%d.alpha", predFold)));
		bw = new BufferedWriter(fw);
		for (int d = 0; d < D; d++) {
			//for (int c = 0; c < 1; c++) {
			//bw.write(docsC0[d] + " ");
		...
		fw = new FileWriter(new File(outputDir, baseName+String.format(".lda.pred.%d.delta", predFold)));
		bw = new BufferedWriter(fw);
		for (int z = 0; z < Z; z++) {
			bw.write(""+z);
		...
		fw = new FileWriter(new File(outputDir, baseName+String.format(".lda.pred.%d.deltaBias", predFold)));
		bw = new BufferedWriter(fw);
		for (int z = 0; z < Z; z++) {
			bw.write(""+z);
		...
		fw = new FileWriter(new File(outputDir, baseName+String.format(".lda.pred.%d.lambda", predFold)));
		bw = new BufferedWriter(fw);
		...
		try {
			FileOutputStream fileOut = new FileOutputStream(new File(outputDir, baseName + String.format(".lda.pred.%d.ser", predFold)));
