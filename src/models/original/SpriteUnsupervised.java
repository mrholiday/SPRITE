 ...
 * and has a 0-mean prior (instead of a prior based on supervision)
 */
public class SpriteUnsupervised extends TopicModel implements Serializable {
	...
	public SpriteUnsupervised(int z, double sigmaAlpha0, double sigmaA0, double sigmaAB0, double sigmaW0, double sigmaWB0,
...
//						updateGradient(iter, minZ, maxZ, minW, maxW);
	...
	public void writeOutput(String filename, String outputDir) throws Exception {
		...
		FileWriter fw = new FileWriter(new File(outputDir, baseName + ".unsupervised.assign"));
		...
		fw = new FileWriter(new File(outputDir, baseName + ".unsupervised.beta"));
		...
		fw = new FileWriter(new File(outputDir, baseName+".unsupervised.betaB"));
		bw = new BufferedWriter(fw);
		for (int z = 0; z < Z; z++) { 
			for (int c = 0; c < Cph; c++) {
				bw.write(betaB[z][c]+" ");
		...
		fw = new FileWriter(new File(outputDir, baseName+".unsupervised.omega"));
		bw = new BufferedWriter(fw);
		for (int w = 0; w < W; w++) {
		...
		fw = new FileWriter(new File(outputDir, baseName+".unsupervised.omegaBias"));
		bw = new BufferedWriter(fw);
		for (int w = 0; w < W; w++) {
			String word = wordMapInv.get(w);
			bw.write(word);
		...
		fw = new FileWriter(new File(outputDir, baseName+".unsupervised.alpha"));
		bw = new BufferedWriter(fw);
		for (int d = 0; d < D; d++) {
			//for (int c = 0; c < 1; c++) {
			//bw.write(docsC0[d] + " ");
		...
		fw = new FileWriter(new File(outputDir, baseName+".unsupervised.delta"));
		bw = new BufferedWriter(fw);
		for (int z = 0; z < Z; z++) {
			bw.write(""+z);
		...
		fw = new FileWriter(new File(outputDir, baseName+".unsupervised.deltaBias"));
		bw = new BufferedWriter(fw);
		for (int z = 0; z < Z; z++) {
			bw.write(""+z);
		...
		fw = new FileWriter(new File(outputDir, baseName+".unsupervised.lambda"));
		bw = new BufferedWriter(fw);
		...
		try {
			FileOutputStream fileOut = new FileOutputStream(new File(outputDir, baseName + ".unsupervised.ser"));
