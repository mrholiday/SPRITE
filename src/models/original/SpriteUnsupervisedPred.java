 ...
 * but has a Gaussian prior whose mean is zero (ignores hashtag-based
 * gun control stance and state-level gun ownership rates).
 */
public class SpriteUnsupervisedPred extends TopicModel implements Serializable {
	...
	public double lambda1; // Controls weight of document score 2
	...
	public SpriteUnsupervisedPred(int z, double sigmaAlpha0, double sigmaA0, double sigmaAB0, double sigmaW0, double sigmaWB0,
			...
			String prefix, double stepA0, int seed0, int numThreads0, int predFold0) {
		Z = z;
...
//						updateGradient(iter, minZ, maxZ, minW, maxW);
	...
	public void doSamplingIteration(int iter) {
		...
		*/
		...
		System.out.println("lambda = "+ lambda0 + " " + lambda1);
	...
	public void writeOutput(String filename, String outputDir) throws Exception {
		...
		FileWriter fw = new FileWriter(new File(outputDir, baseName + String.format(".unsupervised.pred.%d.assign", predFold)));
		...
		fw = new FileWriter(new File(outputDir, baseName + String.format(".unsupervised.pred.%d.beta", predFold)));
		...
		fw = new FileWriter(new File(outputDir, baseName+String.format(".unsupervised.pred.%d.betaB", predFold)));
		bw = new BufferedWriter(fw);
		for (int z = 0; z < Z; z++) { 
			for (int c = 0; c < Cph; c++) {
				bw.write(betaB[z][c]+" ");
		...
		fw = new FileWriter(new File(outputDir, baseName+String.format(".unsupervised.pred.%d.omega", predFold)));
		bw = new BufferedWriter(fw);
		for (int w = 0; w < W; w++) {
		...
		fw = new FileWriter(new File(outputDir, baseName+String.format(".unsupervised.pred.%d.omegaBias", predFold)));
		bw = new BufferedWriter(fw);
		for (int w = 0; w < W; w++) {
			String word = wordMapInv.get(w);
			bw.write(word);
		...
		fw = new FileWriter(new File(outputDir, baseName+String.format(".unsupervised.pred.%d.alpha", predFold)));
		bw = new BufferedWriter(fw);
		for (int d = 0; d < D; d++) {
			//for (int c = 0; c < 1; c++) {
			//bw.write(docsC0[d] + " ");
		...
		fw = new FileWriter(new File(outputDir, baseName+String.format(".unsupervised.pred.%d.delta", predFold)));
		bw = new BufferedWriter(fw);
		for (int z = 0; z < Z; z++) {
			bw.write(""+z);
		...
		fw = new FileWriter(new File(outputDir, baseName+String.format(".unsupervised.pred.%d.deltaBias", predFold)));
		bw = new BufferedWriter(fw);
		for (int z = 0; z < Z; z++) {
			bw.write(""+z);
		...
		fw = new FileWriter(new File(outputDir, baseName+String.format(".unsupervised.pred.%d.lambda", predFold)));
		bw = new BufferedWriter(fw);
		...
		try {
			FileOutputStream fileOut = new FileOutputStream(new File(outputDir, baseName + String.format(".unsupervised.pred.%d.ser", predFold)));
