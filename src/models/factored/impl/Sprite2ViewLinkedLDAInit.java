 ...
 * across both views.
 * 
 * Initialized with \omega from LDA samples.
 * 
 * @author adrianb
 *
 */
public class Sprite2ViewLinkedLDAInit extends SpriteFactoredTopicModel {
	/**
	 * 
	 */
	private static final long serialVersionUID = -8356557959212672700L;
	
	public Sprite2ViewLinkedLDAInit(SpriteThetaPrior[] thetaPriors0,
	...
	private static Tup3<Factor[], SpriteThetaPrior[], SpritePhiPrior[]> buildPriors(int Z, int C,
		...
		Factor[] factors = new Factor[] {new InitOmegaFactor(C, new int[] {0, 1}, new int[] {Z, Z}, 1.0, true,
										 ...
										 false, false, "supertopic", false, true, true, 1000.0, initOmegaPath)};
...
//				 false, false, "supertopic", false, 1.0)};
		...
		SpritePhiPrior phiPrior1 = new SpritePhiPrior(factors, Z, new int[] {0}, initDeltaBias, sigmaDeltaBias, true);
		SpritePhiPrior phiPrior2 = new SpritePhiPrior(factors, Z, new int[] {1}, initOmegaBias, sigmaOmegaBias, true);
...
//		SpritePhiPrior[]   ppriors = {new SpritePhiPrior(factors, Z, 0, initOmegaBias, sigmaOmegaBias)};
	...
	public static void main(String[] args) {
		...
		if (p != null) {
			...
			topicModel = new Sprite2ViewLinkedLDAInit(graph._2(), graph._3(), graph._1(), p.numThreads, p.step);
