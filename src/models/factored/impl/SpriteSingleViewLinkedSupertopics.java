...
/**
 * Topic model with single latent supertopic factor for one view.
 * Delta/Beta ensure that each supertopic has a single child, and
 * Alpha is constrained to be positive.  This should be equivalent
 * to LDA.
 * 
 * @author adrianb
 *
 */
public class SpriteSingleViewLinkedSupertopics extends SpriteFactoredTopicModel {
	/**
	 * 
	 */
	private static final long serialVersionUID = -8356557959212672700L;
	
	public SpriteSingleViewLinkedSupertopics(SpriteThetaPrior[] thetaPriors0,
	...
	private static Tup3<Factor[], SpriteThetaPrior[], SpritePhiPrior[]> buildPriors(int Z, int C,
		...
		SpritePhiPrior phiPrior1 = new SpritePhiPrior(factors, Z, new int[] {0}, initOmegaBias, sigmaOmegaBias, true);
	...
	public static void main(String[] args) {
		...
		if (p != null) {
			...
			topicModel = new SpriteSingleViewLinkedSupertopics(graph._2(), graph._3(), graph._1(), p.numThreads, p.step);
